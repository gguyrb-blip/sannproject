  ...
  return (
    <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-14 pt-4 pb-16 lg:pb-20">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 lg:gap-3.5">
        {GALLERY_IMAGES.map((g, i) => (
          <Link
            href="/gallery"
            key={g.label}
            className="relative rounded-sann-md overflow-hidden aspect-[3/4] group block"
          >
            <div
      ...
      <div className="text-center mt-7">
        <Link
          href="/gallery"
          className="inline-flex items-center gap-1.5 text-sann-red font-medium text-sm hover:gap-2.5 transition-all"
        >
          View full gallery — room by room →
        </Link>
      </div>
    </div>
  );
